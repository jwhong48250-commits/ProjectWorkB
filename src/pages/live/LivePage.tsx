...
import { endWorkspaceMeeting, fetchWorkspaceMeetingDetail } from "../../api/meetings";
import { getCurrentWorkspaceId } from "../../utils/workspace";
import { getMicEnabled, getSelectedCameraId, getSelectedMicId } from "../../utils/deviceSettings";
...
// ── Speaker metadata ─────────────────────────────────────────────────────
const SPEAKER_PALETTE = ["#6366f1", "#f59e0b", "#10b981", "#ef4444", "#8b5cf6", "#06b6d4", "#f97316", "#ec4899"];
...
export default function LivePage() {
    ...
    const fallbackMeeting = MEETINGS.find((m) => m.id === meetingId) ?? MEETINGS[0];
    const [meeting, setMeeting] = useState<Meeting>(() => readMeetingSnapshotForRoute(meetingId) ?? fallbackMeeting);
    ...
    // STT WebSocket hook
    const { wsStatus, liveText, diarization, errorMsg, micOn, toggleMic, stopMeeting } = useLiveSTT(meetingId, {
    ...
    const showEndingModal = (isEndingMeeting && wsStatus !== "error") || pendingNavigateToNotes;
    ...
    function pushLeaveGuardState() {
        ...
        window.history.pushState(
            {
                ...currentState,
                idx: typeof currentState.idx === "number" ? currentState.idx + 1 : currentState.idx,
                key:
    ...
    const elapsedSec = meeting.startAt
        ? Math.max(0, Math.floor((nowMs - new Date(meeting.startAt).getTime()) / 1000))
        : 0;
        ...
        <div className="flex h-full bg-background">
                    ...
                    <div className="max-w-2xl mx-auto">
                        ...
                        {errorMsg && (
                            <div className="mb-4 p-3 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 flex items-center gap-2">
                                ...
                                <p className="text-mini text-red-700 dark:text-red-400">{errorMsg}</p>
                            </div>
                        )}
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            {wsStatus === "connecting" && (
                                <div className="flex flex-col items-center justify-center py-12 gap-3">
                                    ...
                                    <p className="text-mini text-muted-foreground">STT 스트림 연결 중...</p>
                                </div>
                            )}

                            {/* 빈 상태 */}
                            {displaySegments.length === 0 && wsStatus === "connected" && !liveText && (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className="text-mini text-muted-foreground">{timeLabel}</span>
                                            </div>
                            ...
                            {liveText && wsStatus === "connected" && (
                                <div className="flex gap-3 p-3 rounded-lg bg-card border border-border">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="text-sm font-medium text-foreground">인식 중</span>
                                        </div>
                            ...
                            {(wsStatus === "connected" || wsStatus === "finalizing") && !liveText && (
                                <div className="flex gap-3 p-3 rounded-lg bg-card border border-border">
            ...
            {auxPanel !== null && (
                <aside className="hidden lg:flex flex-col w-72 xl:w-80 shrink-0 border-l border-border bg-card overflow-hidden">
                    <div className="flex-1 overflow-y-auto p-3">{renderAuxPanelContent()}</div>
                </aside>
            )}
                    ...
                    <div className="w-full max-w-sm rounded-2xl border border-border bg-popover shadow-xl overflow-hidden">
                        <div className="flex items-center gap-2 px-5 py-4 border-b border-border">
                            ...
                            <h2 id="meeting-ending-title" className="text-sm font-semibold text-foreground">
                                {pendingNavigateToNotes ? "회의 요약 생성 중..." : "회의 종료를 처리하고 있습니다"}
                            </h2>
                        </div>
