...
import { fetchWorkspaceMembers, type WorkspaceMemberApiItem } from "../../api/workspaceMembers";
...
const SPEAKER_COLORS = ["#6b78f6", "#22c55e", "#f97316", "#ec4899", "#14b8a6", "#a855f7", "#eab308", "#ef4444"];
...
export default function NotesPage() {
    ...
    const [pendingScrollTarget, setPendingScrollTarget] = useState<"top" | "bottom" | null>(null);
    ...
    const ASR_BASE = (import.meta.env.VITE_ASR_SERVER as string | undefined) ?? "http://localhost:8888";
    const visibleUtterances = showAllUtterances ? utterances : utterances.slice(0, INITIAL_VISIBLE_UTTERANCES);
    const hiddenUtteranceCount = Math.max(0, utterances.length - visibleUtterances.length);
    ...
    function moveTranscript(position: "top" | "bottom") {
        ...
        if (position === "bottom" && !showAllUtterances && utterances.length > INITIAL_VISIBLE_UTTERANCES) {
    ...
    useEffect(() => {
        ...
        const target = pendingScrollTarget === "top" ? transcriptTopRef.current : transcriptBottomRef.current;
    ...
    async function handleModalSave() {
        ...
        const newId = speakerModal.activeTab === "custom" ? null : speakerModal.selectedMemberId;
        if (!newLabel) return;
            ...
            setUtterances((prev) =>
                prev.map((u) => {
                    if (speakerModal.applyAll) {
                        ...
                        return u.seq === speakerModal.seq ? { ...u, speaker_id: newId, speaker_label: newLabel } : u;
                    }
                }),
            );
    ...
    async function saveEditContent(seq: number) {
        ...
        try {
            ...
            setUtterances((prev) => prev.map((u) => (u.seq === seq ? { ...u, content: trimmed } : u)));
            setEditingSeq(null);
            setEditingText("");
        } finally {
                    ...
                    <div className="w-full max-w-sm mx-4 rounded-xl border border-border bg-popover shadow-xl overflow-hidden">
                        ...
                        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-semibold text-foreground">화자 수정</span>
                        ...
                        <div className="flex border-b border-border">
                            {(["member", "custom"] as const).map((tab) => (
                                <button
                                    key={tab}
                                    onClick={() =>
                                        setSpeakerModal((prev) => (prev ? { ...prev, activeTab: tab } : prev))
                                    }
                                    className={[
                            ...
                            {speakerModal.activeTab === "member" ? (
                                members.length === 0 ? (
                                    ...
                                    <div className="flex flex-col gap-1">
                                        {members.map((m) => {
                                            const isSelected = speakerModal.selectedMemberId === m.user_id;
                                            return (
                                                <button
                                                    key={m.user_id}
                                                    type="button"
                        ...
                        <div className="px-4 pb-4 pt-2 border-t border-border space-y-3">
                            <label className="flex items-center gap-2.5 cursor-pointer select-none">
                                <div
                                    onClick={() =>
                                        setSpeakerModal((prev) => (prev ? { ...prev, applyAll: !prev.applyAll } : prev))
                                    }
                                    className={[
                                ...
                                >
                                    {speakerModal.applyAll && <Check size={10} className="text-white" />}
                                </div>
                                <span className="text-sm text-foreground">전체 변경</span>
                                <span className="text-xs text-muted-foreground">(같은 화자의 모든 발화 변경)</span>
                            </label>
                            ...
                            <div className="flex gap-2">
                                ...
                                >
                                    {modalSaving ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />}
                                    변경 저장
                                </button>
                            </div>
                        </div>
                ...
                <section>
                    ...
                    {utterancesLoading && (
                        <div className="flex items-center gap-2 text-sm text-muted-foreground py-6 justify-center">
                            <Loader2 size={16} className="animate-spin" /> 발화 데이터 로딩 중...
                        </div>
                    )}
                    ...
                    {!utterancesLoading && !utterancesError && utterances.length === 0 && (
                        <p className="text-sm text-muted-foreground py-4 text-center">저장된 발화 데이터가 없습니다.</p>
                    )}
                    ...
                    {!utterancesLoading && !utterancesError && utterances.length > 0 && (
                        <div className="flex flex-col gap-2.5">
                            {visibleUtterances.map((u) => {
                                ...
                                const isActive = fullPlaying && fullCurrentTime >= u.start && fullCurrentTime < u.end;
                                return (
                                    <div
                                        key={u.seq}
                                        className={[
                                            ...
                                            {isEditMode && editingSeq === u.seq ? (
                                                ...
                                                <p
                                                    className={[
                                                        ...
                                                        isEditMode ? "cursor-text hover:bg-muted/30" : "cursor-default",
                                                    ].join(" ")}
                                                    title={isEditMode ? "클릭해서 텍스트 수정" : undefined}
                                                    onClick={isEditMode ? () => startEditContent(u) : undefined}
                                                >
                                                    {u.content}
                                                </p>
                                            )}
